 ...
 * COOP
 * CEnter
 * Esta pantalla sirve para hacer la recarga de linea de credito
 * Rutas:
 * `${environment.apiUrl}/corp/empresas/list/ifis`,
 * `${environment.apiUrl}/corp/creditoPersonas/listar/recargar/lineasCreditos`,
 * `${environment.apiUrl}/corp/creditoPersonas/recargar/lineasCreditos`,
 * `${environment.apiUrl}/corp/creditoPersonas/actualizar/recargar/lineasCreditos/${data.id}`,
 * `${environment.apiUrl}/corp/creditoArchivos/view/creditos/preaprobados/negocios/${id}`
 */

@Component({
  selector: 'app-upload-lineas-creditos',
  templateUrl: './recargar-lineas-creditos.component.html',
  styleUrls: ['./recargar-lineas-creditos.component.scss']
})
export class RecargarLineasCreditos implements OnInit, OnDestroy {
  ...
  negarRecargaLineaCredito(id) {
    this._cargarCreditosNegocios.actualizarRecargaLinea({id, state: 0}).subscribe(info => {
      this.obtenerListaRecargaLineasCredito();
  ...
  procesarRecargaLineaCredito(id) {
    this._cargarCreditosNegocios.actualizarRecargaLinea({id, estado: 'Procesar'}).subscribe(info => {
      this.obtenerListaRecargaLineasCredito();
